...
- Voice transcript extraction (raw-audio mode): the main model prefixes replies with a `<transcript>` block, peeled by `_peel_transcript` in `app/api/chat.py`. Deliver it ONLY in the final `done` SSE payload — a mid-stream `transcript` event + immediate placeholder rewrite made voice replies hang mid-stream (user-reported; exact mechanism unconfirmed). Don't reintroduce mid-stream UI updates there.
