...
- **Mic input cuts mid-utterance were traced to Chromium's WebRTC audio processing (noiseSuppression/autoGainControl), not our capture code** (2026-07-04) — confirmed by the user: cuts persisted even after moving hands-free capture off ScriptProcessorNode to AudioWorklet (which fixed a *different*, real bug: main-thread jank dropping capture callbacks), and Windows' own mic test tool (which bypasses browser APM entirely) never showed the issue. `core.js`'s `micAudioConstraints()` now sets `noiseSuppression`/`autoGainControl` to `false` — keep `echoCancellation: true` only, since `narration.js`'s barge-in logic depends on it to avoid speaker bleed-through false-triggering an interrupt.
