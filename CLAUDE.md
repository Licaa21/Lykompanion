...
Win32 gotchas hit during development — keep all in mind if the overlay looks wrong again:
- **`WS_EX_LAYERED` is required, but only bare** — pywebview's `transparent=True` alone (`SupportsTransparentBackColor` + `DefaultBackgroundColor = Transparent` in `winforms.py`) is NOT sufficient for real desktop-composited transparency on a top-level window; without `WS_EX_LAYERED` set on the hwnd, DWM never engages per-pixel compositing for it and it shows WebView2's default opaque **white** background instead. But `WS_EX_LAYERED` must never be paired with `SetLayeredWindowAttributes`/`UpdateLayeredWindow` (the classic "flat alpha bitmap" APIs) — doing that switches the window into legacy flat-blend mode and made the *whole screen render as an opaque dark tint* instead (an earlier, worse bug than the white-screen one). The correct combination, used by Electron/CEF-style transparent overlays on Windows 8+: set `WS_EX_LAYERED` and stop there — DWM then pulls per-pixel alpha directly from the window's own GPU swap chain / DirectComposition surface (which is exactly how WebView2 renders). This is what `_apply_overlay_base_styles` does now, applied once at window `shown` (before first paint) and never toggled — `WS_EX_TRANSPARENT` (click-through, toggled by `_make_overlay_click_through_setter`) is unrelated and doesn't need `WS_EX_LAYERED` itself, but sharing the same window means both live there.
