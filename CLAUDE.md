...
- **Maximized frameless window showed a faint desktop-colored gap at edges/corners** (2026-07-05) — Win11 rounds top-level window corners (`DWMWA_WINDOW_CORNER_PREFERENCE`) by default; our maximize is a JS snap-to-workarea (`init.js`), not native `WindowState=Maximized`, so the OS never suppresses rounding. Fixed in `run_app.py`'s `_remove_window_border()` (already cleared the accent border via DWM attr 34) by also setting attr 33 to `DWMWCP_DONOTROUND`.
