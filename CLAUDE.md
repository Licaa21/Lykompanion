...
- **Mic input cuts mid-utterance were traced to Chromium's WebRTC noiseSuppression, not our capture code** (2026-07-04) — confirmed by the user: cuts persisted even after moving hands-free capture off ScriptProcessorNode to AudioWorklet (which fixed a *different*, real bug: main-thread jank dropping capture callbacks), and Windows' own mic test tool (which bypasses browser APM entirely) never showed the issue. `core.js`'s `micAudioConstraints()` sets `noiseSuppression: false`. Keep `echoCancellation: true` (narration.js's barge-in logic needs it) and `autoGainControl: true` (turning it off broke hands-free VAD entirely — voice.js's amplitude-threshold gate is calibrated assuming AGC-normalized levels, so raw un-boosted mic input fell below the threshold and hands-free silently stopped detecting speech).
