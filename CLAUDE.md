...
- "Reply freezes mid-stream" root cause: any `renderChatLog()` during streaming (chat switch, modals) wipes `chatLog` and detaches the live bubbles — the stream keeps writing into detached nodes and the UI looks hung. Both stream loops re-attach via `assistantEl.isConnected` checks + a tail re-render; keep that guard when touching streaming code.
