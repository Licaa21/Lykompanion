...
- Overlay build traps: MSVC rejects a function-local struct default-init referencing a function-local enum (move to namespace scope); a stray `"` in PATH breaks `vcvars64.bat` (build.cmd strips quotes — keep that). `overlay/Lykompanion-overlay.exe` is intentionally committed (force-tracked via `overlay/.gitignore`'s `!Lykompanion-overlay.exe`) so users without VS Build Tools still get a working overlay — don't "fix" the recurring dirty-diff by gitignoring/untracking it (tried and reverted 2026-07-05). Root cause was link.exe stamping a build timestamp into the PE header on every rebuild even with unchanged source; fixed via `/Brepro` on both the `cl` and `/link` lines in `build.cmd` for a reproducible build.
