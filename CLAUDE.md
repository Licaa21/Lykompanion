...
- **Mic input dropping consonants (e.g. "What's" losing its "t's") was Chromium's WebRTC `echoCancellation` pipeline** (2026-07-05) — it bundles a transient/click suppressor meant to filter keyboard-typing noise during calls, which reproducibly misfires on short unvoiced plosive/fricative sounds (t, k, p, s). Found only after ruling out (in order): Bluetooth/wireless-dongle duplex issues, WebView2-hosting quirks (repro'd identically via dev-server-in-a-plain-browser-tab, which pointed back at our own code/constraints), and a real but separate bug (see below). `core.js`'s `micAudioConstraints()` now has `echoCancellation: false, noiseSuppression: false, autoGainControl: true`. `autoGainControl` must stay on: the hands-free VAD gate (`voice.js`'s `computeAmplitude > vadThreshold/127`) is calibrated assuming AGC-normalized levels. Losing AEC did surface the expected side effect: narration's speaker bleed-through now reaches the mic unfiltered and was false-triggering `narration.js`'s barge-in (`stopNarration()`'s hard pause sounds like a pop) — mitigated with `voice.js`'s `BARGE_IN_THRESHOLD_MULTIPLIER` (3x the normal VAD threshold, just for the isNarrating check) since bleed-through is quieter at the mic than the user's own voice.
...
- Chirp3 HD narration can pop mid-sentence — confirmed not a bug here (one API call per sentence, no server-side chunking/concatenation, WAV wrapping in `chirp3.py` is correct); it's a known artifact of the Chirp3 HD model itself. Mitigated (not eliminated - it's upstream) by `app/services/tts/__init__.py`'s `_postprocess()`: single-sample impulse declick, peak normalization (consistent loudness across providers/voices), plus fade-in/out (was fade-in only). Runs on every provider's output, pure-Python O(n) over the PCM samples, ~35ms for a 5s clip — negligible next to the TTS network call itself.
