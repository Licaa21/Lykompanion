...
- **Hands-free mic cuts mid-utterance: real root cause was a channel-downmix regression from the ScriptProcessorNode→AudioWorkletNode migration** (2026-07-05), found only after ruling out Bluetooth/wireless-dongle duplex issues, WebView2-hosting quirks, and browser audio processing (none of it — repro'd identically in a plain browser tab via the dev server, which is what pointed back at our own code). `createScriptProcessor(4096, 1, 1)` forced a true mono downmix of the mic input; `AudioWorkletNode` with only `channelCount: 1` does NOT downmix (default `channelCountMode` is `"max"`) — a stereo capture device would leave channel 1 silently discarded (`mic-worklet-processor.js` only reads `inputs[0][0]`) instead of merged in, producing intermittent gaps if the reliable signal wasn't on channel 0. Fixed by adding `channelCountMode: "explicit"` to the `AudioWorkletNode` options in `voice.js`'s `startLiveMic()`. `core.js`'s `micAudioConstraints()` still has `noiseSuppression: false` from an earlier (unconfirmed, but reasonable to keep) hardening attempt — keep `echoCancellation`/`autoGainControl` true (barge-in logic and VAD threshold calibration depend on them respectively).
