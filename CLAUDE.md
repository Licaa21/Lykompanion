...
**One small native window per widget** (currently `toasts` and `game-state`), NOT one shared full-screen window — a full-screen transparent WebView2 window proved unreliable across three straight win32/DWM exstyle attempts (dark tint → opaque white → dark tint again; see "history" below), so the design switched to small per-widget windows, which are a far more common/better-tested case for real WebView2 transparency. Each is created in `run_app.py`'s `main()` (only when the setting is on at launch — enabling needs a restart, disabling hides content live via `overlay_enabled` polling in `overlay.js`, but the native windows themselves stay alive until the app restarts) — frameless/transparent/on-top, sized+positioned individually (`OVERLAY_WIDGETS` list: element id, width, height, default screen corner), made click-through + hidden from Alt-Tab via win32 exstyles (`_apply_overlay_base_styles` for the one-time per-window WS_EX_LAYERED/TOOLWINDOW/NOACTIVATE + `DwmExtendFrameIntoClientArea` setup on `shown`, `_make_overlay_click_through_setter` for the toggleable WS_EX_TRANSPARENT bit — applied to each window AND its own WebView2 child hwnds, re-swept 2s after `shown`). Every widget window loads `web/overlay.html?widget=<id>` + `web/js/overlay.js` (standalone, NOT part of the `web/js/app/*.js` chain; the same JS file runs in every widget window and gates its widget-specific logic on the `?widget=` query param; token via `?token=` everywhere since EventSource can't set headers).
...
**Transparency win32 history** (why the architecture is per-widget-window, not full-screen) — three straight attempts at exstyle/DWM permutations on a single full-screen overlay window all failed: (1) `WS_EX_LAYERED` + `SetLayeredWindowAttributes` → whole screen rendered as an opaque dark tint (that pairing forces legacy flat-alpha blending, wrong for WebView2's GPU-swap-chain rendering); (2) dropping `WS_EX_LAYERED` entirely → opaque white (WebView2's default background, since DWM then never engages per-pixel compositing at all); (3) bare `WS_EX_LAYERED` (no legacy attribute APIs, the theoretically-correct Windows 8+ technique) plus `DwmExtendFrameIntoClientArea(hwnd, {-1,-1,-1,-1})` ("sheet of glass", the older Aero-glass mechanism) together → still an opaque dark tint. If the current small-window architecture *also* turns out not to render transparently, the next fallback (not yet attempted) is dropping WebView2 for the overlay's rendering entirely and drawing the toasts/game-state panel with GDI+/Direct2D directly via ctypes and the classic `UpdateLayeredWindow` per-pixel-alpha-bitmap technique — more code, no HTML/CSS, but the long-established, unambiguously reliable way to get a real transparent overlay on Windows, independent of whatever WebView2/DWM is or isn't doing under the hood. `_apply_overlay_base_styles` still applies the attempt-3 combination (`WS_EX_LAYERED` bare + full-glass extend-frame) per widget window — untouched by the architecture change, since it might matter less (or work fine) at small window sizes even though it didn't at full-screen.
