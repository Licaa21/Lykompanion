...
Two win32 gotchas hit during development, both fixed — keep both in mind if the overlay looks wrong again:
- **Whole screen tinted dark instead of transparent**: caused by `_make_overlay_click_through_setter` adding `WS_EX_LAYERED` + `SetLayeredWindowAttributes` to get click-through. Don't — `WS_EX_TRANSPARENT`/`WS_EX_TOOLWINDOW`/`WS_EX_NOACTIVATE` don't need `WS_EX_LAYERED` at all, and adding it overrides WebView2's own DirectComposition-based per-pixel transparency (the thing that actually makes `transparent=True` work — see `winforms.py`'s `SupportsTransparentBackColor`/`DefaultBackgroundColor` path), forcing legacy flat-alpha layered compositing where WebView2's transparent regions render as opaque black instead.
