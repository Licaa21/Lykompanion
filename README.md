...
| `GAME_STATE_MAX_STALE_SECONDS` | Safety net: if a poll window keeps getting skipped (no OCR change, no meaningful visual diff) for this many seconds since the last structuring pass actually ran for the tracked process, force one through anyway on whatever's on screen. Guards against a real state change both signals missed - e.g. a death/game-over screen that reads as visually similar to the prior frame in a small diff thumbnail - which would otherwise leave stale tracker values (health, "in combat", etc.) displayed indefinitely while the player sits on that screen. Should generally be set higher than `GAME_STATE_POLL_INTERVAL_SECONDS`. Default `120`, `0` = disable. |
