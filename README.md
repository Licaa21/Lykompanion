...
| `OVERLAY_ENABLED` | `false` by default. Launches the native in-game overlay (`overlay/overlay.exe`) while a game is tracked, drawing the game-state panel plus reply/reminder toasts on top of the game (borderless/windowed only — exclusive fullscreen can't show it). Press **Ctrl+Shift+O** in-game to move widgets and change opacity/accent color; the layout persists to `%LOCALAPPDATA%\Lykompanion\overlay_layout.json`. Windows only, and requires the built `overlay.exe` (`overlay/build.cmd`, needs VS Build Tools). |
