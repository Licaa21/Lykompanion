...
| `POST /api/game-art/{process}/fetch` | Fetches (or returns cached) cover art + title for a process — Steam store search, then IGDB, then an LLM+web-search title lookup as a last resort. Cached to `data/game_art.json`. |
