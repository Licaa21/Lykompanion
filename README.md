...
- **Audio device selection** — pick which **microphone** captures your voice (push-to-talk + hands-free) and which **output device** narration/voice replies play through, in Settings → Voice. The choice is saved per-device (localStorage), applied via `getUserMedia` device constraints and `HTMLAudioElement.setSinkId` (output routing falls back to the system default where `setSinkId` isn't supported).
