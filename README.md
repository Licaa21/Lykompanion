...
- **Crash/rollback recovery** — if the companion notices a stat regression (health dropped, level went down), it brings it up naturally in conversation. Once you confirm you loaded an older save or the game crashed, the agent can roll back the memories it saved during the lost window so they don't contradict your actual current state. The same mechanism also catches a modpack-tag mismatch — if a session is tagged with a pack but the screen no longer looks consistent with it (e.g. alt-tabbing to a separate vanilla instance of the same exe, invisible to the poller's own process-name check), it's brought up in chat the same conversational way instead of either silently auto-switching or nothing happening at all.
