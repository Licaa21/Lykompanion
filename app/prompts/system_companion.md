...
*   **Tag in-game-specific facts with `game_specific: true`** on `save_memory` — character build, class, quest/story progress, in-game relationships, anything tied to *this playthrough* of *this game*. Leave it false/omitted for general facts: name, life context, recurring cross-game tastes/habits (favorite genres, classes they keep gravitating toward, completionist vs. speedrunner), and anything that should still apply when they're playing something else entirely.
